...
One such important application is separating weeds, crops and dirt in aerial drone images of a field.
Applying the SegNet architecture to this field of crop segmentation could open up for smart agriculture that allows a farmer to localize the use of fertilizer and pesticides with positive environmental and economic consequences.
...
In image segmentation, this problem of robust, generalizable models can be even more difficult as the classes that are to be predicted often have very different frequencies of occurence: Some -- often background classes like dirt in this task -- are very prevalent while other classes appear rarely among the pixels but are still important to the task. 
This imbalance must be addressed in the training and evaluation for effective segmentation also for this task which contains 3 classes: weeds, crops, and dirt.
\subsection{The Data: Brazilian Sugar Canes and Danish Corn}
The primary data set in question consists of two large, high resolution orthomosaic RGB images of a Brazilian sugar cane field\(^\text{Note 1}\) \cite{brazil}.
...
The network was trained for 4000 epochs each consisting of mini batches of size 3.
...
In the main dataset, \(\sim 93 \%\) of pixels belong to the dirt-class which arguably is the least interesting and initial tests saw the network behave too much as a baseline; simple features in early layers which classified almost everything as dirt were not penalized enough and learning was not stable. 
...
USC is the first work on the dataset from Universidad de Catarina \cite{brazil} and Cellari is the work from the project supervisor \url{https://cellari.io}.
...
\\
\\
The test of the performance of the same, main SegNet model on the small, novel \textbf{corn field data} set gives disappointing results: Only a test F1 score of 0.34 which outperforms the baseline but tells a story of overfitting with a train F1 of 0.79 on this small dataset of 20 512x512 train images.
...
It was learned that adressing class imbalance between the \textit{dirt} class and weed and crops by weighting the cross entropy loss was important for high performance and that this Deep Learning model required more than the 20 images from the corn dataset to give accurate results.
...
\bibliography{refs}
\subsection{Appendix}
\footnotetext[1]{Aerial image: http://www.lapix.ufsc.br/wp-content/uploads/2019/05\\/sugarcane2.png\\
		Ground truth: http://www.lapix.ufsc.br/wp-content/uploads/2019/05\\/crop6GT.png}
\footnotetext[2]{From \url{http://www.taranis.ag}, 2/01-20: "Taranis uses sophisticated computer vision, data science and deep learning algorithms to effectively monitor fields (...) and can detect when a weed emerges."  }
\begin{figure}[h!]
	\centering
...
\begin{figure}[h!]
	\centering
%	\includegraphics[width=0.7\linewidth]{anno2}
