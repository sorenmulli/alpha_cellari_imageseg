...
Farmers, and later, biologists, have spent centuries manually seperating crops from weeds to optimize the year's harvest. Agriculture can be made more efficient by using automization to seperate the crops from the weeds and the dirt. \\
On this poster, you can expect to learn about the implementation of SegNet for pixelwise crop classification on a drone image. The results of this implementation is also shown as well as the problems with such data and what can be done to improve it for future implementations
